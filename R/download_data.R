...
#' Download data from the UNGA-DM Database
#'
#' @description This function allows you to download data from the UNGA-DM
...
#'
#' @details The UNGA-DM Database API has a rate limit, so this function
...
#'   \code{authenticate()}.
#' @param dataset A string. The name of a dataset in UNGA-DM Database. Use
...
#'
#' @export
download_data <- function(session, dataset, filters = NULL, variables = NULL) {
  route <- get_api_route(dataset = dataset)
  parameters <- filters
